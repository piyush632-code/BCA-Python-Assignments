...
# Try load; if not found, fall back to the actual file in this workspace
if not os.path.exists(DATA_FILE) and os.path.exists("ssample_student_score.csv"):
    DATA_FILE = "ssample_student_score.csv"
...
# normalize spaces to underscores (e.g. 'roll no' -> 'roll_no')
...
# Drop entirely-empty trailing columns (fix for CSVs with extra commas),
# then drop rows with missing required values.
...
class StudentManager:
    ...
    def add_student_record(self, row):
        ...
        if key not in self.students:
            self.students[key] = Student(row["name"], row["roll_no"], row.get("gender", ""))
...
# Save cleaned data
# (we kept this so you have the cleaned CSV used for analysis)
